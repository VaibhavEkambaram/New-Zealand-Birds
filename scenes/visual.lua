...
-----------------------------------------------------------------------------------------------------------------------
	...
	createText(heading,"Visual Index",(fullWidth*0.2),(fullHeight*0.11), native.systemFont, 32 )
-----------------------------------------------------------------------------------------------------------------------
	-- Search Bar
-----------------------------------------------------------------------------------------------------------------------	
 ...
 -----------------------------------------------------------------------------------------------------------------------
	local function onRowTouch( event )
		...
		-- If the menu is cliked then go to the bird scene with the following parameters				
        if event.phase == "press" or event.phase == "tap" then     
			local options = {
				effect = "slideLeft", -- slide left animation with 500ms delay
				time = 500,	
				params = { -- these parameters are used in the next scene to populate the bird information
					id = params.id, -- row identifier
					birdName = params.birdName, -- name of bird (English/Maori)
					birdImage = params.birdImage, -- bird image file path
					birdDescription = params.birdDescription, -- detailed description of bird
					scientificName = params.scientificName, -- scientific name of bird
					status = params.status, -- endemic / native / introduced status
					search = false -- search validation condition
				}			
			}
			-- Go to the bird scene with the above options
			composer.gotoScene( "scenes.bird", options ) 
        end
    end
-----------------------------------------------------------------------------------------------------------------------
	-- Row Render event for table view    
-----------------------------------------------------------------------------------------------------------------------	
	local function onRowRender( event )
		local row = event.row
		local id = row.index
		local params = event.row.params
		local rowHeight = row.contentHeight -- Define row height
		local rowWidth = row.contentWidth -- Define row width
		local rowImage = display.newImage(params.birdImage,midpointX,rowHeight-265) -- Create an image object
			row:insert(rowImage) -- Add the above image to the row
			-- Set the anchor points of the image
			rowImage.anchor = 0.5 
			rowImage.anchorY = 0
			-- Set the scale of the image
			rowImage:scale( 0.5, 0.5)
	end	
-----------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------	
	...
	{	
		left = 0, -- set the left most position to the left most position on the screen
		isBounceEnabled = true, -- bounce effect when reaching the top or bottom of the tableView
		top = (fullHeight*0.21),
		height = (fullHeight*0.6896), -- set the height
		width = (fullWidth), -- set the width
	...
	-- select from birds table using name column, sort by name (alphabetical)
	for row in db:nrows("SELECT * FROM birds ORDER BY name ASC") do  
		menuTable:insertRow{
		    rowHeight = (fullHeight*0.22),
			...
			params = { 
				...
				scientificName = row.scientific, -- bird scientific name
