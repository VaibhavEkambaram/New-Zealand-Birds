...
-----------------------------------------------------------------------------------------------------------------------
	...
	createText(heading,"Taxonomic Index",(fullWidth*0.2),(fullHeight*0.11), native.systemFont, 32 )
-----------------------------------------------------------------------------------------------------------------------
	-- Search Bar
-----------------------------------------------------------------------------------------------------------------------	
	...
	local function onRowTouch( event )
        ...
        if event.phase == "press" or event.phase == "tap" then     
			local options = {
				...
				time = 500,	
				params = {
					-- Put these variables as parameters
					id = params.id,
					cat = params.cat,
					search = false,
				}			
			}
			-- go to the birds scene
...
-----------------------------------------------------------------------------------------------------------------------	
	local function onRowRender( event )
		...
		local rowText = display.newText( params.cat,10,0,nil,32)
...
----------------------------------------------------------------------------------------------------------------------- 	
	...
	-- select from taxonomic table, sort by ascending order 
	for row in db:nrows("SELECT * FROM taxonomic ORDER BY id ASC ") do 
		menuTable:insertRow{
